...
Redaction deletes the thing a model needs to reason about ("the same person
...
* A **token** stands for one identifier of one subject **within one session**.
  The same subject is the same token throughout a session and a different token
...
* The **token map** holds each identifier encrypted under the subject's own data
  key in :class:`trustkernel.kernel.key_custody.KeyCustody`. Destroying that key makes every
...
* **Restoration** needs a custody credential with ``decrypt``. The vault never
  decides who may see identity; :class:`trustkernel.kernel.privacy_pipeline.PrivacyGate`
