...
side effect is in a third system (a payment, an email, a customer record in a
...
def schema_statements(dialect: Dialect, schema: str = "trustkernel") -> list[str]:
    ...
    statements += [
        ...
        f"CREATE INDEX IF NOT EXISTS trustkernel_evidence_kind ON {prefix}evidence (kind)",
    ]
    return statements
...
class SqlDatabase:
    ...
    def __init__(self, connect: Callable[[], Any], dialect: Dialect, schema: str = "trustkernel",
...
def open_sqlite(path: str = ":memory:", schema: str = "trustkernel",
...
def open_postgres(dsn: str, schema: str = "trustkernel",
                  *, evidence_token: str | None = None) -> SqlDatabase:
    ...
    try:
        ...
        raise ImportError(
            "The PostgreSQL backend requires the 'postgres' extra: pip install 'trustkernel[postgres]'"
        ) from exc
