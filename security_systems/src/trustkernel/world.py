"""A scenario world in which every mediator is real and every control can be removed.

A world wires the kernel's actual components together around one domain pack: key
custody and encrypted records (data plane), the context gate and privacy pipeline
(data mediator), the model registry (intelligence plane admission), delegation
(authority), the accountable executor with Ed25519 approvals (power mediator), the
bounded review queue (human oversight), and the evidence ledger with a signing notary
(evidence). Model outputs and human decisions are scripted; storage and cryptographic
components execute locally.

Nothing in this module names a domain. The cast (subjects and their records,
principals and their roles, agents, register resources, trusted document sources)
and the fixtures the attack suites use are read from ``worlds/<id>/world.yaml``;
the policy is read from the pack that file names. A new domain is a new directory,
not a new class. See ``docs/WRITING_A_WORLD.md``.
...
from .agents import ModelTurn, Router
from .kernel.accountable_action import ActionClass
from .kernel.delegation import AuthorityScope, DelegationAuthority, DelegationPolicy, RootGrant
from .kernel.disclosure import (
    ALL_CHECKS,
    ConsentRegister,
...
from .kernel.encrypted_records import EncryptedRecordSource, GovernedVectorIndex
from .kernel.evidence import EvidenceLedger
from .kernel.evidence_notary import EvidenceNotary, verify_against_checkpoint
...
from .kernel.grant_delegation import GrantDelegationService
from .kernel.key_custody import KeyCustody
from .kernel.model_registry import (
    ManifestPublisher,
    ModelAttestationDenied,
    ModelRegistry,
    artifact_digest,
)
from .kernel.pack_floor import GovernedPack, load_governed_pack
from .kernel.privacy_pipeline import ErasureService, ModelContext, PrivacyGate
from .kernel.privacy_vault import TokenVault
from .kernel.review_queue import BoundedReviewQueue, ReviewRefused, ReviewState

ROOT = Path(__file__).resolve().parents[2]
WORLDS = ROOT / "worlds"
EVIDENCE_TOKEN = "scenario-world-evidence-writer"
...
class WorldSpecError(ValueError):
    """Raised when a world.yaml is incomplete or refers to something its pack lacks."""


@dataclass(frozen=True)
class WorldSpec:
    """Everything a world declares, read once from ``world.yaml`` and never mutated."""
    ...
    # -- loading ------------------------------------------------------------------
    @classmethod
    def load(cls, name_or_path: str | Path) -> WorldSpec:
        return _load_spec(str(name_or_path))

    def _get(self, *path: str) -> Any:
        value: Any = self.raw
        for key in path:
            if not isinstance(value, dict) or key not in value:
                raise WorldSpecError(f"{self.world_id}: world.yaml is missing {'.'.join(path)}")
            value = value[key]
        return value
    ...
    def pack_path(self) -> Path:
        return self.directory / self._get("pack")

    @property
    def malicious_pack_path(self) -> Path:
        return self.directory / self._get("malicious_pack", "path")

    @property
    def now(self) -> float:
        return float(self.raw.get("now", 1_800_000_000.0))
    ...
    def source_keys(self) -> dict[str, str]:
        return self._get("source_keys")

    @property
    def injection(self) -> re.Pattern:
        return re.compile("|".join(self._get("injection_patterns")), re.IGNORECASE)

    @property
    def scenario(self) -> dict:
        return self._get("scenario")

    def fixture(self, *path: str) -> Any:
        return self._get("scenario", *path)

    def field(self, role: str) -> str:
        """The pack field that plays ``role`` (routine, sensitive, identity, ...) here."""
        return self._get("scenario", "fields", role)

    def to_summary(self) -> dict:
        return {"world_id": self.world_id, "title": self.title,
                "description": self.raw.get("description", "").strip(),
                "subjects": len(self.subjects), "principals": len(self.principals),
                "agents": len(self.agents), "resources": len(self.resources),
                "pack": str(self.pack_path.relative_to(ROOT)) if self.pack_path.is_relative_to(ROOT)
                else str(self.pack_path)}


@cache
def _load_spec(name_or_path: str) -> WorldSpec:
    candidate = Path(name_or_path)
    if candidate.suffix in (".yaml", ".yml") and candidate.is_file():
        path = candidate.resolve()
    elif (candidate / "world.yaml").is_file():
        path = (candidate / "world.yaml").resolve()
    else:
        path = WORLDS / name_or_path / "world.yaml"
    if not path.is_file():
        known = ", ".join(available_worlds()) or "none"
        raise WorldSpecError(f"no world {name_or_path!r} (known: {known})")
    raw = yaml.safe_load(path.read_text(encoding="utf-8"))
    if not isinstance(raw, dict) or not raw.get("world_id"):
        raise WorldSpecError(f"{path}: a world must be a mapping with a world_id")
    return WorldSpec(str(raw["world_id"]), str(raw.get("title", raw["world_id"])), path.parent, raw)


def available_worlds() -> list[str]:
    return sorted(p.parent.name for p in WORLDS.glob("*/world.yaml"))
...
class CredentialedRegister(CaseRegister):
    ...
    def __init__(self, cases: dict, *, credential: str, world: ScenarioWorld) -> None:
...
class ScenarioWorld:
    def __init__(self, world: str | Path | WorldSpec = "devtools",
                 controls: Iterable[str] = ALL_CONTROLS, *, pack_path: Path | None = None,
                 model_endpoint: str | None = None,
                 approval_seed: bytes | None = None) -> None:
        self.spec = world if isinstance(world, WorldSpec) else WorldSpec.load(world)
        spec = self.spec
        ...
        self.now = spec.now
        self.pack: GovernedPack = load_governed_pack(pack_path or spec.pack_path,
                                                     floor=self.on("pack_floor"))
        self.profile = self.pack.profile
        self.policy = self.profile.disclosure
        self.observed = WorldObservations()
        # Seeds are namespaced by world so two worlds never share key material.
        seed = lambda label: hashlib.sha256(f"{spec.world_id}:{label}".encode()).digest()  # noqa: E731

        # Evidence plane
        self.ledger = EvidenceLedger(EVIDENCE_TOKEN)
        self.notary = EvidenceNotary(seed=seed("world-notary"))
        self.checkpoints: list = []

        # Data plane: custody, encrypted records, tokens, derived index
        self.custody = KeyCustody(master_seed=seed("world-custody"),
        ...
        self._erase_key = self.custody.register_principal("data-protection-officer", ["erase", "backup"])
        ...
        self.vault = TokenVault(self.custody, self._gate_key, key=seed("world-vault"))
        self.vector_index = GovernedVectorIndex(self.custody, writer_credential=self._gate_key,
                                                reader_credential=self._gate_key)
        index = spec.raw.get("vector_index") or {}
        if index:
            for subject, row in spec.subjects.items():
                self.vector_index.add(subject, index["namespace"], row[index["field"]])

        # Authority plane: grant, declassification, and delegation authorities
        self.grants = GrantAuthority(key_id="data-grants-1", secret=f"{spec.world_id}-grant-secret")
        self.exchange = GrantAuthority(key_id="delegation-exchange-1",
                                       secret=f"{spec.world_id}-exchange-secret")
        self.declassifier = DeclassificationAuthority(key_id="declassification-1",
                                                      secret=f"{spec.world_id}-declass-secret")
        ...
        self.publisher = ManifestPublisher(seed=seed("model-risk-office"))
        ...
        for endpoint, manifest in sorted(self.pack.model_manifests.items()):
            artifact = f"weights:{manifest['model_id']}".encode()
            self.registry.register(self.publisher.issue(
                endpoint=endpoint, zone=self.policy.endpoints[endpoint], provider=manifest["provider"],
                model_id=manifest["model_id"], artifact=artifact, capabilities=manifest["capabilities"],
                allowed_classes=manifest["allowed_classes"], allowed_purposes=manifest["allowed_purposes"],
                expires_at=self.now + manifest["approval_days"] * 86_400))
            self.runtimes[endpoint] = (artifact_digest(artifact), manifest["model_id"])
        # Endpoints that exist on the network but hold no approved manifest.
        for endpoint, model_id in (spec.raw.get("unattested_runtimes") or {}).items():
            self.runtimes[endpoint] = (artifact_digest(f"weights:{model_id}:unapproved".encode()), model_id)
        self.model_endpoint = model_endpoint or spec.fixture("endpoints", "local")
        self.router = Router(self.policy, spec.raw.get("router_costs") or {})
        ...
        self.register = CredentialedRegister(
            {rid: {"status": status, "version": 1} for rid, status in spec.resources.items()},
            credential=self._register_key, world=self)
        # The approval signing key is generated fresh for every world and lives only in
        # this process. Rule 1 depends on its secrecy, so it must NOT be a constant an
        # attacker who reads this source could reconstruct. The executor is handed only
        # the matching *public* verification key. A test that needs a reproducible key
        # passes ``approval_seed``. (The notary, publisher, and custody keys stay seeded:
        # they make the *evidence* reproducible and are verification keys, not the
        # secret Rule 1 rests on.)
        self.audience = spec._get("executor_audience")
        self.approvals = AsymmetricApprovalAuthority(self.audience, seed=approval_seed)
        ...
        self.review = BoundedReviewQueue(
            ...
            escalation_role=review["escalation_role"], reviewers=spec.principals,
    ...
    def value(self, subject: str, field_name: str) -> str:
        """Ground truth: the plaintext a falsifier checks for in what a model saw."""
        return self.spec.subjects[subject][field_name]
    ...
    def admit_document(self, source: str, text: str, signature: str) -> dict:
        ...
        key = self.spec.source_keys.get(source)
        authentic = key is not None and hmac.compare_digest(
        ...
        markers = [m.group(0) for m in self.spec.injection.finditer(text)]
    ...
    def sign_document(self, source: str, text: str) -> str:
        return hmac.new(self.spec.source_keys[source].encode(), text.encode(), hashlib.sha256).hexdigest()
    ...
    def grant(self, *, holder: str, purpose: str, subjects: Iterable[str], fields: Iterable[str],
              ...
              issued_by: str | None = None) -> DisclosureGrant:
        fields = tuple(fields)
        self._requests += 1
        return self.grants.issue(
            ...
            issued_by=issued_by or self.spec.fixture("data_officer"), now=self.now,
    ...
    def propose(self, *, requester: str, operation: str, resource: str, to_status: str,
        ...
        self._requests += 1
        current = (self.register.get(resource) if resource in self.spec.resources
                   else {"status": from_status or "", "version": 1})
    ...
    def review_and_approve(self, proposal: ActionProposal, *, reviewer: str,
        ...
        role = self.spec.principals.get(reviewer, "")
    ...
    def context_authority_live(self, grant: DisclosureGrant) -> None:
        ...
        Exact-proposal approval stops substitution of *what* is done, but it does not by
        itself carry the authority of the *read* the proposal was built on. A proposal
        drafted over a context that was later revoked, expired, or had its consent
        withdrawn must not still execute. The gate re-runs its own currency, consent,
        and scope checks over the grant's declared subjects and fields through the
        side-effect-free :meth:`authorize_only`; if any now fails the write is refused.
        """
        try:
    ...
    def execute(self, proposal: ActionProposal, approval: Approval | None, *, presenter: str | None = None,
        ...
        try:
            ...
            if model_endpoint is not None and self.on("model_attestation"):
                self.attest(model_endpoint, purpose=self.spec.fixture("action", "attest_purpose"),
                            fields=(self.spec.fixture("action", "attest_field"),))
    ...
    def protected_values(self, subject: str, fields: Iterable[str]) -> list[str]:
        return [self.spec.subjects[subject][name] for name in fields]

    def identity_values(self) -> list[str]:
        return [row[name] for row in self.spec.subjects.values() for name in self.pack.identity_fields]

    def action_root(self, principal: str | None = None, *, operations: Iterable[str] | None = None,
                    resources: Iterable[str] = ("*",), expires_in: float = 3600.0) -> RootGrant:
        """The institutional grant an agent's action chain must descend from."""
        operations = tuple(operations or (self.spec.fixture("action", "operation"),))
        return RootGrant(principal=principal or self.spec.fixture("agent"),
                         owner=self.spec.fixture("action", "root_owner"),
                         expires_at=self.spec.now + expires_in,
...
__all__ = ["ALL_CONTROLS", "DATA_CONTROLS", "EXECUTION_CONTROLS", "MEDIATOR_OF", "PLATFORM_CONTROLS",
           "ScenarioWorld", "WorldObservations", "WorldSpec", "WorldSpecError", "available_worlds"]
