...
attackers of increasing adaptivity against a real scenario world and scores
every episode by actual forbidden effects, not by denial messages.

Attacker tracks (each is labelled honestly for what it is)
----------------------------------------------------------
``static``   The fixed grammar corpus in :mod:`trustkernel.redteam`, replayed under a
             fixed seed. No adaptation. It is included so the others have something
             to beat.
...
* Reward comes from :func:`trustkernel.redteam.execute_attack`'s observation-based
  ``violated`` verdict, read from :class:`trustkernel.world.WorldObservations`
...
def _run_spec(spec: dict, controls, rng: random.Random, world: WorldSpec) -> EpisodeResult:
    """Run one attack spec against a fresh world and score it by real effect.

    The success signal is :func:`trustkernel.redteam.execute_attack`'s ``violated``
    ...
    """
    violated, code = execute_attack(ScenarioWorld(world, controls), spec, rng)
    forbidden: list[ForbiddenOutcome] = []
    if violated:
...
def _spec_for(move: str, rng: random.Random, world: WorldSpec) -> dict:
    """A random spec over the world's declared attack vocabulary, widened past the grammar's."""
    vocab = world.fixture("attack")
    return {"move": move,
            "fields": rng.sample([*vocab["fields"], *vocab.get("extra_fields", ())], rng.randint(1, 3)),
            "subjects": rng.sample(sorted(world.subjects), rng.randint(1, 3)),
            "purpose": rng.choice([*vocab["purposes"], *vocab.get("extra_purposes", ())]),
            "endpoint": rng.choice(list(vocab["endpoints"]))}
...
def run_track(name: str, *, budget: int, seed: int, controls, moves, world: WorldSpec) -> TrackReport:
    report = TrackReport(name)
    rng = random.Random(seed)
    if name == "static":
        from .redteam import GrammarAttacker
        attacker = GrammarAttacker(world, seed=seed)
        for _ in range(budget):
            spec = attacker.next_attack()
            spec["move"] = spec["move"] if spec["move"] in moves else moves[0]
            report.record(spec["move"], _run_spec(spec, controls, rng, world))
        return report
    if name == "random":
        for _ in range(budget):
            spec = _spec_for(rng.choice(moves), rng, world)
            report.record(spec["move"], _run_spec(spec, controls, rng, world))
        return report
    if name == "bandit":
        ...
        for _ in range(budget):
            move = bandit.choose()
            spec = _spec_for(move, rng, world)
            result = _run_spec(spec, controls, rng, world)
...
def run_live_track(*, budget: int, seed: int, controls, moves, world: WorldSpec,
    ...
    try:
        ...
        attacker = OllamaRedTeam(world, model=model, host=host)
...
class AdaptiveReport:
    ...
    def to_dict(self) -> dict:
        return {"world": self.world, "budget_per_track": self.budget, "seed": self.seed, "split": self.split,
...
def run_adaptive(*, world: WorldSpec | str = "devtools", budget: int = 200, seed: int = 20260921, remove=(),
                 split: str = "dev", include_live: bool = False,
                 prove_attacker: bool = False) -> AdaptiveReport:
    world = world if isinstance(world, WorldSpec) else WorldSpec.load(world)
    ...
    tracks = [run_track(name, budget=budget, seed=seed, controls=controls, moves=moves, world=world)
              for name in ("static", "random", "bandit")]
    if include_live:
        tracks.append(run_live_track(budget=budget, seed=seed, controls=controls, moves=moves, world=world))
    positive = None
    if prove_attacker:
        ...
        proven = run_track("bandit", budget=budget, seed=seed, controls=weakened,
                           moves=("altered_proposal", "forged_approval", "self_approval"), world=world)
        positive = {"control_removed": "execution_mediator", **proven.to_dict()}
    return AdaptiveReport(budget, seed, world.world_id, removed, split, tracks, positive)
