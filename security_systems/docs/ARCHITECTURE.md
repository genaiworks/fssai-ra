...
A row is **load-bearing** when the attack fails with the control on, succeeds with it off, and fails again once it's restored. Four rows are not load-bearing in any shipped world, and the tests pin them:
...
The kernel knows nothing about services, students, or builds. A world supplies two files:
