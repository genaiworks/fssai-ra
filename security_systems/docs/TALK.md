...
python demo.py --no-color > /tmp/rehearsal.txt   # about 2 s; keep this as the offline fallback
```

Use `trustkernel demo --pause 0.8` on stage. It's the same output, paced for reading.

## The arc (about 20 minutes)
...
| 6 | Remove one control at a time | `--scene 6` | 87 worlds in about a second. 25 of 29 controls are load-bearing. The four that aren't are named defence in depth, and removing each pair together lets the attack through. | 2:00 |
| 7 | Clone it, break it | slide with the repo URL | `python demo.py`, then `trustkernel falsify`. A falsifier is one function. | 1:30 |
...
- **"Does it only work for this demo?"** Run `trustkernel demo --world education`. It's the same kernel, a different domain, and the same verdicts, and `tests/test_reference_equivalence.py` pins that world to the original implementation's recorded figures.
