...
from trustkernel.adaptive_attack import (
    DEV_MOVES,
    HELD_OUT_MOVES,
    BanditAttacker,
    _run_spec,
...
from trustkernel.world import ALL_CONTROLS, WorldSpec, available_worlds

SPEC = WorldSpec.load("devtools")


@pytest.mark.parametrize("world", available_worlds())
def test_no_track_finds_a_forbidden_outcome_with_all_controls_on(world):
    report = run_adaptive(world=world, budget=150, split="all", seed=20260921)
...
def test_bandit_concentrates_budget_on_rewarded_moves():
    ...
    for _ in range(60):
        move = bandit.choose()
        result = _run_spec(_spec_for(move, rng, SPEC), controls, rng, SPEC)
...
def test_live_track_reports_not_run_without_a_runtime():
    report = run_live_track(budget=5, seed=1, controls=list(ALL_CONTROLS),
                            moves=DEV_MOVES, world=SPEC, host="http://127.0.0.1:1")  # unreachable port
