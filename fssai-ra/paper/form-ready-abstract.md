...
**Reference implementation:** https://github.com/genaiworks/fssai-ra release v1.0.0
...
Eight of eight ablated controls restored their matching harm, for authority coverage of 1.0. Twenty five conformance checks pass on two backend profiles. A 32 caller race against one approved request produced one mutation, one receipt, one intent, one outcome, and 31 replay replies. This covers one process, not distributed linearizability. In total, 187 deterministic tests run offline. Generated result files keep paper and code figures aligned.
