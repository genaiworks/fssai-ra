...
Privacy pipeline. The target profile replaces declared identifiers with scoped tokens and encrypts fields under per-person keys. Tokens are not anonymity: rare attributes and free text can still identify people. Erasure checks cover supplied copies, not every backup or released output. The router ranks approved endpoints; the gate checks grant, purpose, consent and zone before access. A signed manifest and a runtime-reported digest do not prove which weights a dishonest runtime loaded. Outputs inherit input restrictions; identity restoration needs recipient entitlement. An opt-in HTTP profile binds these modules; durable custody and deployment qualification remain open. No claim of universal privacy follows from the disclosure fixtures.
