"""Deterministic build of the Trust by Construction V18 manuscripts.
...
ABSTRACT_REFS = [3, 26, 9, 7, 14, 29, 25, 32, 41]
...
ALT = {
    "fig1-enforcement-path.png":
        "Three boxes left to right: model and inputs, authority service, protected systems. "
        "The authority service reads current trusted state. An AI monitor may only restrict "
        "authority; a named operator is the only path that restores it.",
    "fig2-five-step-contract.png":
        "Eight task stages, each marked as a boundary, beside a panel listing the five-step "
        "enforcement contract applied at every marked boundary.",
    "fig3-authority-envelope.png":
        "Four nested sets: workload passport ceiling, task contract, effective envelope A(s) "
        "and admitted operations O(s, m, r), illustrating that admitted operations stay inside "
        "the effective envelope.",
    "fig4-delivery-revocation.png":
    ...
    "fig5-agent-graph.png":
        "A coordinator and three agents in a row; each agent's own scope is allowed, while the "
        "composed path runs from a protected source to a public sink and is rejected by the "
        "declared-graph check before the last agent acts.",
    "fig6-release-channel.png":
        "Two bar charts: distinguishable release traces of 16, 1 and 0 bits under three policies, "
        "and the share of adversarial tasks escalated, 0, 98.4 and 98.4 per cent.",
}
...
def build_docx(kind, items, full, path):
    ...
    doc.core_properties.title = ("Trust by Construction for Secure Agentic Workflows "
                                 "in Learning Institutions")
    doc.core_properties.subject = "V-18 | " + ("Full technical paper" if full
                                               else "Extended abstract")
    doc.core_properties.version = "18"
...
def html_document(kind, items, full):
    ...
    title = "Trust by Construction V18" + ("" if full else " Extended Abstract")
...
def build(kind):
    ...
    stem = ("Trust_by_Construction_V18" if full
            else "Trust_by_Construction_V18_Extended_Abstract")
