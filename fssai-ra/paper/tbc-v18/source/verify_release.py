"""Checks the V18 release before it is distributed.
...
Rebuilds both manuscripts into a temporary directory and verifies:
  ...
  5. the distributed files match V18_SHA256SUMS.txt.
...
print("verifying the V18 release")
with tempfile.TemporaryDirectory() as tmp:
    ...
    check("full paper carries its figures and tables",
          full["figures"] == 6 and full["tables"] == 5)
...
sums = RELEASE / "V18_SHA256SUMS.txt"
if sums.exists():
    mismatched = []
    ...
    check("distributed files match V18_SHA256SUMS.txt", not mismatched,
          ", ".join(mismatched))
else:
    print("  [skip] V18_SHA256SUMS.txt not written yet")
