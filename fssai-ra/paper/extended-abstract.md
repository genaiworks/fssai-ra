...
Across the corporate and healthcare packs, **48 of 48 hostile data flows were contained**. Conventional access control, with signed class-cleared grants and an output check on the model's claimed label, **contained 16**. **All 14 checks were load-bearing**, **9,168 read and release configurations showed zero violations**, and 5 of 5 legitimate flows completed. Single-step enumeration missed one defect: stateful testing, **400 sequences and 16,000 operations** against an independent reference model, found that a summary drafted before consent was withdrawn could still be released. Release now rechecks consent, revocation, and expiry; the sequences agree.
