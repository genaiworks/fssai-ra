...
| The paper cannot drift from the code | Generated figures; the paper and the deck are parsed and checked against them | `tests/test_paper_alignment.py` | Checks numbers and required limit statements, not the correctness of the argument |
...
| Public claim | Enforcement or mechanism | Executable evidence | Limit |
|---|---|---|---|
| One authority kernel is reusable across different secure-data workflows | Six independently reported packs: student support, academic-record correction, corporate-confidential data, healthcare-record access, consumer finance, and government benefits; 55,440 total bounded configurations, 180/180 hostile scenarios contained, 58/58 benign tasks completed, zero unauthorized mutations | `tests/test_domain_packs.py`; `fssaira profiles --verify`; `evaluation/results/v1.0.0-domain-pack-matrix.json` | All packs and fixtures were authored here. They demonstrate reuse of the authority mechanism, not sector compliance, privacy, fairness, clinical safety, or production readiness |
...
| This found a real gap in our own contract | The first run reported **18 of 28** requirements with a failure test written in prose and bound to nothing. Most had tests; nothing connected them, so deleting one removed a governance claim silently. One control — ET-2 — had no check at all and now has `CF-ET-03` | `evaluation/results/v1.0.0-contract-coverage.json` | The current figure is 34 machine-verified, 3 attested, 0 unverified. That it was ever 18 is the finding |
...
| Consent, revocation, and expiry stop the release of anything already derived | Release rechecks live state for every subject and every grant that fed the output | `test_release_rechecks_consent_and_revocation_after_the_read`; `test_random_operation_sequences_agree_with_the_reference_model` | Found by stateful random testing after single-step enumeration had passed; sequences are random, not exhaustive |
| Disclosure state survives restarts and concurrent replicas | Transactional SQLite or PostgreSQL store; each decision is one transaction; per-holder locking for emergency access | `test_sql_store_survives_restart_and_forgets_nothing`; `test_independent_processes_share_one_store_without_violating_either_invariant` | One host, local SQLite WAL, finite schedules; PostgreSQL locking implemented but not exercised under failover or load |
| Refusals do not reveal whether a subject exists | The record source is read only after authorization | `test_record_sources_are_read_only_after_authorization_so_existence_does_not_leak` | Timing differences are not measured |
| Grants from an institutional authorization server are verified strictly | Asymmetric keys, issuer, audience, required claims, one disclosure detail, grant bound to token | `test_grant_tokens_are_rejected_when_anything_is_wrong`; `test_the_api_registers_token_grants_only_for_their_holder` | Keys are generated in the test; no named identity provider has been qualified |
| Value-level labels are more precise without becoming weaker | Labels recomputed from issued values; unissued values refused; unnamed verbatim values force the session label | `test_value_level_labels_release_what_session_labels_would_refuse`; `test_an_omitted_source_over_labels_and_the_evidence_says_so` | Depends on a trusted orchestrator; paraphrase is not detected |
| Every disclosure check is load-bearing | Each of fourteen checks is removed in turn | `test_every_disclosure_check_is_load_bearing` | Load-bearing against the declared flows only |
