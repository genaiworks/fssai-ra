...
| Adoption planner | Turn one use case into a target level, applicable controls, a first sprint and stop conditions | `fssaira framework plan`; [guide](ADOPT.md); `tests/test_adoption_plan.py` | A documented judgement to review, not a certification |
| Injection benchmark | Measure attack success and utility per defence with a worst-case agent (a model-independent bound) and local models | `fssaira bench injection`; [guide](INJECTION_BENCH.md); `tests/test_injection_bench.py` | Synthetic tasks authored here, AgentDojo design; oracle review is an idealisation |
| MCP gate | Put the tool-supply rules in front of any MCP host: scan, named lock, rug-pull quarantine, untrusted-chain blocking, argument scope, session contract, exact-action approval with signed approvers, local and remote (Streamable HTTP) servers, a shared authenticated gateway with one gate per session, hash-chained receipts | `fssaira mcp scan/lock/serve/verify`; [guide](MCP_GATE.md); `tests/test_mcp_gate.py` | gateway expects TLS in front; does not attest server behaviour or see content reaching the model by other paths |
