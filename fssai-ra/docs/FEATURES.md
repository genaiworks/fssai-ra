...
| MCP gate | Put the tool-supply rules in front of any MCP host: scan, named lock, rug-pull quarantine, untrusted-chain blocking, argument scope, session contract, exact-action approval with signed approvers, local and remote (Streamable HTTP) servers, hash-chained receipts | `fssaira mcp scan/lock/serve/verify`; [guide](MCP_GATE.md); `tests/test_mcp_gate.py` | one host session per gate process; does not attest server behaviour or see content reaching the model by other paths |
