...
| Decision receipts | `decision_receipts`, `verify_receipts`, `prune_receipts` | Every model request (allowed, denied, or blocked before mediation, including Guardian refusals) and every fan-in decision writes a receipt binding policy version, Passport digest, task epoch, source lineage, artifact digest, recipient, outcome and stable refusal code. Each MAC covers the previous one, so deletion or reordering fails `verify_receipts`; export its `head` to detect tail truncation. A refused request's untrusted destination text is never stored, only its request digest. Receipts carry identifiers and digests, not record text; only operator or monitor authority reads them. Pruning verifies the chain first, removes only a prefix and moves a stored anchor |
| Shared budget reservations | `reserve_budget`, `settle_reservation`, `cancel_reservation`, `reservations` | A trusted scheduler reserves from the one task and workload balance before dispatching parallel workers. A reservation bound to an agent (`agent=`) meters that agent's charges: reserved dimensions are charged once, against the reservation, and the worker cannot spend past it (`RESERVATION_EXHAUSTED`). Settlement of a metered reservation uses measured use and refunds the rest; cancellation never refunds budget already spent. An unbound reservation settles with reported use. A reservation closes at most once |
| Declared task graph (P10) | `admit_graph`, `retract_graph`, `task_graph` | Opt-in per task. Once declared, an agent acts only after operator admission as a node, and a message crosses only an admitted (source, target, channel) edge; replanning is a further admission and retraction only removes. Receipt rechecks the edge, so retraction also stops messages already queued on it. Tasks without a declaration behave as before |
| Fan-in gate | `accept_result` | Before a coordinator accepts a worker result: object MAC and exact expected digest, a live producer in the same task, the current epoch, admitted node and edge when a graph is declared, the coordinator's right to receive, labels within its data rights, unquarantined sources and a permitted destination. The transfer is charged to the shared context budget like a received message, and refusals are receipted |
| Task revocation epoch | `TrustRuntime.revoke_task` (named operator, reason required), `Guardian.revoke_task` | Raises the task epoch so queued proposals and approvals, stale leases, escrows and later release chunks fail revalidation; disclosed bytes are not recalled |
...
| Minimum review time | `TrustRuntime(min_deliberation_seconds=...)`, `manual_queue` | An approval issued sooner than the floor after its proposal is refused with `REVIEW_DEFERRED_TO_MANUAL` and recorded on a manual route; a later valid approval resolves the entry. The floor is measured from proposal creation. The default floor of 0 keeps previous behaviour |
...
child = client.spawn_agent(narrower_scope, model="offline-scripted", zone="local", ttl=60)
message = client.send_message(child["agent"], "internal", summary["artifact"])
```
