...
def figure_architecture(figures: dict) -> tuple[str, str]:
    ...
    body = [
        ...
        text(92, 40, "Sees vault tokens, never identities. May propose and request; "
             "holds no credential or key.", size=10.5, fill=INK_2),
    ]
    ...
    body += [
        ...
        *_lines(290, 256, ("Rechecks policy, identity, version,",
                           ...
                           "exact proposal digest; one write,",
                           "one receipt.")),
        ...
        *_lines(290, 425, ("Encrypted at rest under per-subject keys; erased by destroying "
                           "the key.",
...
STEPS = (
    ("Admit", "BOUNDARY PLANE",
     ...
     "Nothing unauthenticated enters.", "plane"),
    ("Protect", "GOVERNED DATA PLANE",
     ...
     "Identities never reach a model.", "mediator"),
    ...
    ("Execute", "EXECUTION PLANE",
     ("Recheck policy, identity, version, approval, and",
      "replay; make one write and issue one receipt."),
     "Exactly one checked effect.", "mediator"),
    ("Release", "CONTEXT GATE",
     ("Label the output by everything it used; restore real",
      "values only for a recipient entitled to see them."),
     "Nothing is laundered out.", "mediator"),
    ("Record", "EVIDENCE PLANE",
     ("Bind intent to outcome in an append-only chain that",
      "stores digests, never the protected values."),
     "Every outcome can be reconstructed.", "plane"),
    ("Recover", "RESILIENCE PLANE",
     ("Revoke, reconcile uncertain effects, fall back to",
      "manual work, and erase a subject by destroying its key."),
     "Failure is safe; erasure is real.", "plane"),
)
